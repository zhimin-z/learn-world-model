...
**[DreamDojo](https://arxiv.org/abs/2602.06949)**（NVIDIA et al., 2025）从另一个角度解决数据匮乏问题：与其等待机器人数据积累，不如直接从**大规模人类第一视角视频**中学习。DreamDojo 整理了 44k 小时第一视角人类操作视频（DreamDojo-HV），覆盖约 96 倍于现有公开数据集的技能种类和 2000 倍的场景数量。人类与机器人之间存在形态差异，但物理交互规律一致，这是知识迁移的基础。缺乏动作标注是人类视频的天然缺陷，DreamDojo 用**连续潜在动作**（continuous latent actions）作为统一的代理动作，以自监督方式从帧间差异中提取语义动作，绕开了标注问题。在少量目标机器人数据上微调后，模型展现出跨环境零样本泛化能力。通过 Self-Forcing 蒸馏，最终模型可在 640×480 分辨率下以 10.81 FPS 实时推理，支持遥操作、策略评估和模型驱动的规划。
